...
router.beforeEach(async(to, from, next) => {
  ...
  if (hasToken) {
    if (to.path === '/login') {
      ...
      const hasGottenUserInfo = store.getters.userCode
      if (hasGottenUserInfo) {
        next()
      } else {
        try {
          // get user info
          await store.dispatch('user/getAuthInfo')
          const accessibleRoutes = await store.dispatch('authorization/getDynamicRoutes')
